...
import {
  ...
  Image, Modal, ActivityIndicator, Platform, Linking,
} from 'react-native';
...
export default function VariationDetailScreen() {
  ...
  const handleEmail = () => {
    if (!variation) return;
    const varId = formatVariationId(variation.sequenceNumber);
    const subject = `${varId} — ${variation.title}`;

    const lines: string[] = [
      `VARIATION CLAIM: ${varId}`,
      `Title: ${variation.title}`,
      variation.projectName ? `Project: ${variation.projectName}` : '',
      `Status: ${capitalize(variation.status)}`,
      `Estimated Value: ${formatCurrency(variation.estimatedValue)}`,
      '',
      '--- DETAILS ---',
      `Captured: ${formatDateTime(variation.capturedAt)}`,
      `Source: ${capitalize(variation.instructionSource)}`,
      variation.instructedBy ? `Instructed By: ${variation.instructedBy}` : '',
      variation.referenceDoc ? `Reference: ${variation.referenceDoc}` : '',
      variation.latitude ? `GPS: ${formatCoordinates(variation.latitude, variation.longitude!)}` : '',
      '',
      variation.description ? `--- DESCRIPTION ---\n${variation.description}\n` : '',
      variation.notes ? `--- NOTES ---\n${variation.notes}\n` : '',
      variation.photos.length > 0 ? `--- EVIDENCE ---\n${variation.photos.length} photo(s) captured — see attached or digital record.\n` : '',
      variation.voiceNotes.length > 0 ? variation.voiceNotes
        .filter(vn => vn.transcription)
        .map(vn => `Voice Note: "${vn.transcription}"`)
        .join('\n') : '',
      attachments.length > 0 ? `--- ATTACHMENTS ---\n${attachments.map(a => `• ${a.fileName}`).join('\n')}\nPlease see attached files.\n` : '',
      variation.evidenceHash ? `Evidence Hash: ${variation.evidenceHash}` : '',
      '',
      '---',
      'Generated by Variation Capture · Pipeline Consulting Pty Ltd',
    ].filter(Boolean);
        ...
        */}
        ...
        {(isOffice || variation.status === VariationStatus.CAPTURED) && (
          <View style={styles.actionsSection}>
            ...
            <Pressable style={styles.actionRow} onPress={handleEmail}>
              <Ionicons name="mail-outline" size={22} color={colors.accent} />
              <Text style={styles.actionRowText}>Email Variation</Text>
            </Pressable>
            <View style={styles.actionDivider} />
